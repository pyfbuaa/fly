...
class Train:
    ...
    def train(self):
        ...
        print("log.csv is created...")
        logdf = pd.DataFrame(columns=['total_loss', 'rpn_loss_cls', 'rpn_loss_box', 'loss_cls', 'loss_box'])
        ...
        while iter < cfg.FLAGS.max_iters + 1:
            ...
            if iter % (cfg.FLAGS.display) == 0:
                ...
                logdf.loc[iter] = [total_loss, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box]
                print('speed: {:.3f}s / iter'.format(timer.average_time))

            if iter % cfg.FLAGS.snapshot_iterations == 0:
                self.snapshot(sess, iter )

        logdf.to_csv('trainloss.csv')
